...
public class QuestBaseController {
    ...
    public ResponseEntity postObject(@RequestBody QuestBase questBase) {
        try {
            return ResponseEntity.ok(questBaseMapper.toDto(questBaseService.addObject(questBase)));
        } catch (BadRequest e) {
    ...
    public ResponseEntity getAllObjects() {
        return ResponseEntity.ok(questBaseService.getAllObjects().stream().map(questBaseMapper::toDto).collect(Collectors.toList()));
    }
